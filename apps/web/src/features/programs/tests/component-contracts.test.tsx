...
// The components live one per file under ../components; the source-level
// assertions below run against all of them together, which is what
// ../shared-ui.tsx used to hold in a single file.
function readComponentSources(directory: URL): string {
  return readdirSync(directory, { withFileTypes: true })
    .sort((left, right) => left.name.localeCompare(right.name))
    .map((entry) => entry.isDirectory()
      ? readComponentSources(new URL(`${entry.name}/`, directory))
      : readFileSync(new URL(entry.name, directory), "utf8"))
    .join("\n");
}

const componentSource = readComponentSources(new URL("../components/", import.meta.url));
