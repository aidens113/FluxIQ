...
describe("AutomationProjectTree", () => {
  it("uses role-specific classes instead of cross-role tree button selectors", () => {
    ...
    const rows = readFileSync(fileURLToPath(new URL("../TreeRows.tsx", import.meta.url)), "utf8");
  ...
  it("keeps one controller identity and delegates active-view primary validation", () => {
    const source = readFileSync(fileURLToPath(new URL("../ProjectTree.tsx", import.meta.url)), "utf8");
    const primary = readFileSync(fileURLToPath(new URL("../../hooks/usePrimaryTreeNodeId.ts", import.meta.url)), "utf8");
  ...
  it("does not require timer advancement to dispatch sidebar preview opens", () => {
    const source = readFileSync(fileURLToPath(new URL("../TreeRows.tsx", import.meta.url)), "utf8");
