    ...
    <section className="automation-runs-workspace automation-flow-settings-workspace">
      ...
      {settingsPending ? <div className="automation-runtime-empty"><span aria-hidden className="automation-inline-spinner" />Loading saved Flow settings...</div> : settingsLoadError ? <div className="automation-runtime-empty"><strong>Flow settings are unavailable</strong><span>{settingsLoadError}</span><button className="button" onClick={() => setSettingsLoadRevision((current) => current + 1)} type="button">Retry</button></div> : <SettingsSectionLayout activeSection={activeSection} ariaLabel="Flow settings sections" onActiveSectionChange={setActiveSection} sections={FLOW_SETTINGS_SECTIONS}>
        ...
        <section className="automation-settings-panel automation-settings-panel-wide" id="flow-settings-dependencies">
          ...
          <div className="automation-settings-divider"><strong>Per-request diagnosis limits</strong><span>Every live request is rejected above the Core hard ceiling of 50,000 total tokens.</span></div>
          <div className="automation-settings-inline-fields">
            <label><span>Input tokens</span><input aria-label="Input tokens" max={50000} min={1} step={1} type="number" value={draft.llmMaxInputTokens} onChange={(event) => updateDraft("llmMaxInputTokens", event.target.value)} /></label>
            <label><span>Output tokens</span><input aria-label="Output tokens" max={50000} min={1} step={1} type="number" value={draft.llmMaxOutputTokens} onChange={(event) => updateDraft("llmMaxOutputTokens", event.target.value)} /></label>
          ...
          <div className="automation-settings-inline-fields">
            <label><span>Max calls</span><input aria-label="Max calls" max={1} min={1} step={1} type="number" value={draft.llmMaxCalls} onChange={(event) => updateDraft("llmMaxCalls", event.target.value)} /><small>Diagnosis-only currently permits exactly one call.</small></label>
            ...
            <label><span>Provider retries</span><input aria-label="Provider retries" max={0} min={0} step={1} type="number" value={draft.llmRetryCount} onChange={(event) => updateDraft("llmRetryCount", event.target.value)} /><small>Retries are disabled for diagnosis-only runs.</small></label>
          </div>
        </section>
