    ...
    <section className="automation-runs-workspace automation-flow-settings-workspace">
      ...
      {settingsPending ? <div className="automation-runtime-empty"><span aria-hidden className="automation-inline-spinner" />Loading saved Flow settings...</div> : settingsLoadError ? <div className="automation-runtime-empty"><strong>Flow settings are unavailable</strong><span>{settingsLoadError}</span><button className="button" onClick={() => setSettingsLoadRevision((current) => current + 1)} type="button">Retry</button></div> : <SettingsSectionLayout activeSection={activeSection} ariaLabel="Flow settings sections" onActiveSectionChange={setActiveSection} sections={FLOW_SETTINGS_SECTIONS}>
        ...
        <section className="automation-settings-panel automation-settings-panel-wide" id="flow-settings-dependencies">
          ...
          <div className="automation-settings-divider"><strong>Per-request LLM limits</strong><span>Every live request is rejected above the Core hard ceiling of 64,000 total tokens.</span></div>
          <div className="automation-settings-inline-fields">
            <label><span>Input tokens</span><input aria-label="Input tokens" max={64000} min={1} step={1} type="number" value={draft.llmMaxInputTokens} onChange={(event) => updateDraft("llmMaxInputTokens", event.target.value)} /></label>
            <label><span>Output tokens</span><input aria-label="Output tokens" max={64000} min={1} step={1} type="number" value={draft.llmMaxOutputTokens} onChange={(event) => updateDraft("llmMaxOutputTokens", event.target.value)} /></label>
            <label><span>Total tokens</span><input aria-label="Total tokens" max={64000} min={1} step={1} type="number" value={draft.llmMaxTotalTokens} onChange={(event) => updateDraft("llmMaxTotalTokens", event.target.value)} /></label>
          </div>
          <div className="automation-settings-inline-fields">
