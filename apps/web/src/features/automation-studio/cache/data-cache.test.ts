...
import { AutomationStudioDataCache } from "./data-cache";
...
describe("Automation Studio summary/detail cache", () => {
  ...
  it("stays passive and browser-neutral under the project data platform", () => {
    const source = readFileSync(new URL("./data-cache.ts", import.meta.url), "utf8");

    expect(source).not.toContain("useEffect");
    expect(source).not.toContain("program-api:mutation");
