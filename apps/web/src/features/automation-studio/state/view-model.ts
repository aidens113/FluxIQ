...
function selectionSignature(selection: AutomationSelection | null): string {
  if (!selection) return "";
  return [
    selection.kind,
    selection.id,
...
function collectionIdentitySignature(values: unknown[] | undefined, idKeys: string[], revisionKeys: string[]): string {
...
function objectIdentitySignature(value: unknown, idKeys: string[], revisionKeys: string[], fallbackIndex?: number): string {
...
function indexedStateSourcesSignature(values: NonNullable<BuildNodeStateViewModelInput["indexedStateSources"]>): string {
...
function stateSnapshotShapeSignature(snapshot: StateSnapshot): string {
...
function nestedIdSignature(record: StateSignatureObject, path: string[], idKeys: string[], revisionKeys: string[]): string {
  let current: unknown = record;
  for (const key of path) current = objectRecord(current)?.[key];
  return objectIdentitySignature(current, idKeys, revisionKeys);
}

function arrayCountSignature(record: StateSignatureObject, key: string): string {
  const value = record[key];
  return Array.isArray(value) ? `${key}:${value.length}` : "";
}

function firstSignatureValue(record: StateSignatureObject, keys: string[]): string | undefined {
...
function signaturePrimitive(value: unknown): string {
