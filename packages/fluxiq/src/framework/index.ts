...
export class FluxIQ {
  ...
  constructor(options: FluxIQOptions = {}) {
    ...
    this.paths = {
      ...
      domains: resolveInside(root, options.domainsDir ?? cleanEnv(env.FLUXIQ_DOMAINS_DIR) ?? "domains"),
      domainPrograms: resolveInside(root, options.domainProgramsDir ?? cleanEnv(env.FLUXIQ_DOMAIN_PROGRAMS_DIR) ?? path.join("domains", "programs")),
      domainInputs: resolveInside(root, options.domainInputsDir ?? cleanEnv(env.FLUXIQ_DOMAIN_INPUTS_DIR) ?? path.join("domains", "inputs")),
      domainOutputs: resolveInside(root, options.domainOutputsDir ?? cleanEnv(env.FLUXIQ_DOMAIN_OUTPUTS_DIR) ?? path.join("domains", "outputs")),
      domainConfigs: resolveInside(root, options.domainConfigsDir ?? cleanEnv(env.FLUXIQ_DOMAIN_CONFIGS_DIR) ?? path.join("domains", "configs")),
      domainData: resolveInside(root, options.domainDataDir ?? cleanEnv(env.FLUXIQ_DOMAIN_DATA_DIR) ?? path.join("domains", "data")),
      domainDatabases: resolveInside(root, options.domainDatabasesDir ?? cleanEnv(env.FLUXIQ_DOMAIN_DATABASES_DIR) ?? path.join("domains", "databases")),
