...
export function registerFlowLifecycleEndpoints(dependencies: AutomationStudioApiDependencies): void {
  ...
  registry.register({
    ...
    permission: "flows.write",
    classification: "authoring",
    handler: async (request) => { const payload = request.payload && typeof request.payload === "object" ? request.payload as { projectId?: unknown; flowId?: unknown; version?: unknown; reason?: unknown; authSessionId?: unknown; authorizationPin?: unknown } : {}; return { ok: true, payload: { publication: await service.deprecateFlowPublication({ projectId: String(payload.projectId ?? ""), flowId: String(payload.flowId ?? ""), version: String(payload.version ?? ""), ...(typeof payload.reason === "string" ? { reason: payload.reason } : {}) }) } }; }
  });
  registry.register({
    programId: "automation-studio",
