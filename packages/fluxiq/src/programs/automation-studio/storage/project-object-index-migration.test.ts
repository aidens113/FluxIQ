...
const rootDir = path.join(process.cwd(), ".tmp", "automation-studio-object-index-migration-test");
...
describe("migrateAutomationStudioLegacyObjectIndex", () => {
  ...
  it("imports legacy objects.json metadata into SQL without moving bytes", async () => {
    ...
    await writeLegacyIndex(projectId, [{ sha256: shaOne, mediaType: "application/json", size: 5, owner: { kind: "project" }, relativePath: objectPath.replaceAll(path.sep, "/"), createdAt: 1 }]);
  ...
  it("reports missing legacy object bytes and resumes without duplicating object rows", async () => {
    ...
    await writeLegacyIndex(projectId, [
...
async function writeLegacyIndex(projectId: string, objects: unknown[]): Promise<void> {
