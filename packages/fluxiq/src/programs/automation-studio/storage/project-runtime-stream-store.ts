...
export class AutomationStudioProjectRuntimeStreamStore {
  ...
  async ensureRuntimeFlowProjection(input: { flowId: string; name?: string; now?: number }): Promise<void> {
    ...
    await this.lease.database.run(
      `insert into flows (flow_id, name, scope_kind, visibility, origin, source_mode, status, created_at_ms, updated_at_ms)
       values (?, ?, 'project', 'project', 'runtime_projection', 'visual', 'draft', ?, ?)
  ...
  async upsertRunSummary(summary: AutomationStudioFlowRunSummary): Promise<AutomationStudioFlowRunSummary> {
    await this.lease.database.run(
      `insert into runtime_runs (run_id, flow_id, flow_revision, status, trigger_kind, queued_at_ms, started_at_ms, finished_at_ms, action_count, effect_count, error_count, adaptation_count, last_event_sequence, updated_at_ms)
       ...
       on conflict(run_id) do update set flow_id = excluded.flow_id, status = excluded.status, started_at_ms = excluded.started_at_ms,
         ...
         adaptation_count = excluded.adaptation_count, updated_at_ms = excluded.updated_at_ms`,
      [
  ...
  async putRunDetail(detail: AutomationStudioFlowRunDetail): Promise<AutomationStudioFlowRunDetail> {
    ...
    if (await this.getRunLastSequence(detail.summary.runId) === 0) {
      const events = runtimeEventsFromDetail(detail);
      if (events.length) await this.appendRuntimeEvents({ runId: detail.summary.runId, events });
      await this.upsertRunSummary(detail.summary);
    }
    return detail;
  }
  ...
  async listRuntimeEvents(input: { runId: string; afterSequence?: unknown; limit?: unknown }): Promise<AutomationStudioRuntimeEventPage> {
    ...
    const events = page.events.map((event) => runtimeEventFromChunk(event));
    return { events, nextCursor: page.nextCursor, hasMore: page.hasMore, lastSequence: events.at(-1)?.sequence ?? afterSequence };
  }

  async getRunDetail(runId: string): Promise<AutomationStudioFlowRunDetail | null> {
    const summary = await this.getRunSummary(runId);
    if (!summary) return null;
    return runDetailFromEvents(summary, await this.readAllRuntimeEvents(runId, 5_000));
  }

  async listRunActions(input: { runId: string; limit?: unknown; offset?: unknown }): Promise<AutomationStudioProjectFlowRunActionPage> {
    const limit = clampInteger(input.limit, 1, 100, 50);
    const offset = clampInteger(input.offset, 0, 10_000_000, 0);
    const totalRow = await this.lease.database.get<{ total: number }>("select action_count as total from runtime_runs where run_id = ?", [requiredId(input.runId, "run")]);
    const total = totalRow?.total ?? 0;
    const actions: AutomationStudioFlowRunActionAttemptRecord[] = [];
    let skipped = 0;
    let afterSequence = 0;
    for (;;) {
      const page = await this.listRuntimeEvents({ runId: input.runId, afterSequence, limit: 500 });
      for (const event of page.events) {
        if (event.eventKind !== "action_attempt") continue;
        if (skipped < offset) { skipped += 1; continue; }
        actions.push(event.payload as unknown as AutomationStudioFlowRunActionAttemptRecord);
        if (actions.length >= limit) break;
      }
      if (actions.length >= limit || !page.hasMore || page.lastSequence <= afterSequence) break;
...
type RuntimeRunRow = { run_id: string; flow_id: string; flow_revision: number; status: AutomationStudioFlowRunStatus; queued_at_ms: number; started_at_ms: number | null; finished_at_ms: number | null; action_count: number; effect_count: number; error_count: number; adaptation_count: number; last_event_sequence: number; updated_at_ms: number };
...
type RuntimeEventCandidate = Omit<AutomationStudioRuntimeStreamEvent, "sequence"> & { order: number };
...
function runDetailFromEvents(summary: AutomationStudioFlowRunSummary, events: AutomationStudioRuntimeStreamEvent[]): AutomationStudioFlowRunDetail {
  const envelope = compactJsonObject(events.find((event) => event.eventKind === "run_summary")?.payload) as Partial<AutomationStudioFlowRunDetail>;
  const actions: AutomationStudioFlowRunActionAttemptRecord[] = [];
  const routeDecisions: AutomationStudioRouteDecisionRecord[] = [];
  const subflows: AutomationStudioSubflowExecutionRecord[] = [];
  const recoveryAttempts: AutomationStudioFlowRunRecoveryRecord[] = [];
  const interventions: AutomationStudioFlowIntervention[] = [];
  for (const event of events) {
    if (event.eventKind === "action_attempt") actions.push(event.payload as unknown as AutomationStudioFlowRunActionAttemptRecord);
    else if (event.eventKind === "route_decision") routeDecisions.push(event.payload as unknown as AutomationStudioRouteDecisionRecord);
    else if (event.eventKind === "subflow_execution") subflows.push(event.payload as unknown as AutomationStudioSubflowExecutionRecord);
    else if (event.eventKind === "recovery_attempt") recoveryAttempts.push(event.payload as unknown as AutomationStudioFlowRunRecoveryRecord);
    else if (event.eventKind === "intervention") interventions.push(event.payload as unknown as AutomationStudioFlowIntervention);
  }
  return {
    schemaVersion: "0.1",
    ...envelope,
...
function runtimeEventFromChunk(event: AutomationStudioChunkEvent): AutomationStudioRuntimeStreamEvent {
  return {
...
function runtimeSummaryFromRow(projectId: string, row: RuntimeRunRow): AutomationStudioFlowRunSummary {
  return { schemaVersion: "0.1", runId: row.run_id, flowId: row.flow_id, projectId, flowVersion: String(row.flow_revision), status: row.status, ...(row.started_at_ms !== null ? { startedAt: row.started_at_ms } : {}), ...(row.finished_at_ms !== null ? { finishedAt: row.finished_at_ms } : {}), updatedAt: row.updated_at_ms, routeDecisionCount: 0, subflowEntryCount: 0, actionAttemptCount: row.action_count, interventionCount: 0, adaptationCount: row.adaptation_count, metadata: { effectCount: row.effect_count, errorCount: row.error_count, lastEventSequence: row.last_event_sequence } };
}
