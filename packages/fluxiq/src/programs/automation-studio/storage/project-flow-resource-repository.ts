...
function subflowSummaryOrder(sort: "updated" | "name" | "status" | "role", direction: "asc" | "desc"): string { const dir = directionSql(direction); const column = sort === "name" ? "lower(name)" : sort === "status" ? "status" : sort === "role" ? "role" : "updated_at_ms"; return `order by ${column} ${dir}, subflow_id ${dir}`; }
function instructionSummaryOrder(sort: "updated" | "title" | "status" | "scope" | "priority", direction: "asc" | "desc"): string { const dir = directionSql(direction); const column = sort === "title" ? "lower(i.title)" : sort === "status" ? "i.status" : sort === "scope" ? "coalesce(s.scope_kind, '')" : sort === "priority" ? "i.priority" : "i.updated_at_ms"; return `order by ${column} ${dir}, i.instruction_id ${dir}`; }
