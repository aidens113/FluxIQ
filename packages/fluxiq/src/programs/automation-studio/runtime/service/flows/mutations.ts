...
export class AutomationStudioFlowMutations {
  ...
  async saveFlowRouter(router: AutomationStudioFlowRouter): Promise<AutomationStudioFlowRouter> {
    const ownerFlow = await this.facade.getFlow(router.projectId, router.flowId);
    ...
    const subflows = (await Promise.all(
      (subflowIndex.subflows ?? [])
        ...
        .map((summary) => this.facade.getFlowSubflow(router.projectId, router.flowId, summary.subflowId))
  ...
  async saveFlowSubflow(subflow: AutomationStudioFlowSubflow): Promise<AutomationStudioFlowSubflow> {
    const parentFlow = await this.facade.getFlow(subflow.projectId, subflow.flowId);
    ...
    const existingSubflow = await this.facade.getFlowSubflow(subflow.projectId, subflow.flowId, subflow.subflowId);
    ...
    const declaredGraph = await this.facade.getFlow(persistedSubflow.projectId, persistedSubflow.graphFlowId!).catch(() => null);
    if (declaredGraph) {
    ...
    const router = await this.facade.getFlowRouter(persistedSubflow.projectId, persistedSubflow.flowId);
  ...
  async createFlowSubflow(input: CreateFlowSubflowInput): Promise<AutomationStudioFlowSubflow> {
    const parentFlow = await this.facade.getFlow(input.projectId, input.flowId);
    if (this.flowWriter.persistedFlowRepresentation(parentFlow) === "subflow_graph") {
      throw new Error("Subflows can only be owned by a top-level Flow.");
    }
    const now = Date.now();
  ...
  async deleteCreatedFlowSubflow(projectId: string, flowId: string, subflowId: string, graphFlowId: string | undefined, deleteGraphFlow: boolean): Promise<void> {
    const existing = flowId && subflowId ? await this.facade.getFlowSubflow(projectId, flowId, subflowId).catch(() => null) : null;
  ...
  async getFlowSubflowsForValidation(projectId: string, flowId: string): Promise<AutomationStudioFlowSubflow[]> {
    ...
    return (await Promise.all(
      (index.subflows ?? [])
        ...
        .map((summary) => this.facade.getFlowSubflow(projectId, flowId, summary.subflowId))
    )).filter((subflow): subflow is AutomationStudioFlowSubflow => Boolean(subflow));
  }
}
