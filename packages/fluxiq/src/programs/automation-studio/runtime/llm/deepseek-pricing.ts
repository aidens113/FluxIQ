...
// changes its price list; everything in `deepseek-provider.ts` is Core's own
// contract with the endpoint. They were one file until the cache split arrived
// and pushed it past its line budget, which was the file saying what it had
// become.
 ...
 *
 * DeepSeek prices a cache hit at a tenth of a cache miss, and this is that
 * tenth of the peak miss rate already recorded here. Like the other two it is a
 * dated, provider-owned maintenance input, and it must be reviewed whenever
 * DeepSeek changes model compatibility or pricing.
 *
 ...
 */
export const AUTOMATION_STUDIO_DEEPSEEK_PEAK_CACHE_HIT_INPUT_USD_PER_MILLION_TOKENS = 0.044;
 ...
 * all-miss figure it always got. The rates are scaled by a thousand rather than
 * a hundred because the hit rate has three decimal places; the arithmetic stays
 * integral, and the two rates that existed before produce the same fractions
 * they always did.
 */
export function estimateAutomationStudioDeepSeekCostUsd(inputTokens: number, outputTokens: number, cacheHitInputTokens = 0): number {
  ...
  const missRateThousandths = Math.round(AUTOMATION_STUDIO_DEEPSEEK_PEAK_CACHE_MISS_INPUT_USD_PER_MILLION_TOKENS * 1_000);
  const hitRateThousandths = Math.round(AUTOMATION_STUDIO_DEEPSEEK_PEAK_CACHE_HIT_INPUT_USD_PER_MILLION_TOKENS * 1_000);
  const outputRateThousandths = Math.round(AUTOMATION_STUDIO_DEEPSEEK_PEAK_OUTPUT_USD_PER_MILLION_TOKENS * 1_000);
