...
Before an importing repository relies on the scalable Automation Studio v2 data
path without compatibility feature flags, attach the final certification report
generated by `pnpm studio:certify`. The report records the scale matrix,
24-hour append/subscription soaks, crash recovery, heap retention, query and
payload budgets, backup restore, deterministic compiled-plan replay, and docs
freshness expected by the framework release gate.
...
Studio treats recordings as optional evidence that can help a human or LLM
understand a difficult custom scenario, while durable generated changes move
through Adaptations. Importers should focus on factual event/state quality; they
do not need to call mining or generation stages directly in normal operation.
...
In the web panel, stopping or finalizing a recording updates the recording row
and timeline cache for that recording only. The current UI does not expose a
recording-driven Proposal Generator. Runtime improvements, new subflows, router
changes, instruction suggestions, and recovery changes are reviewed through
Adaptations, with recording IDs attached only as evidence when relevant. When
the web panel deletes multiple recordings from a project selection, it calls the
batch `delete-recordings` endpoint so recording folders, indexes, object
references, and orphaned session directories are cleaned in one grouped runtime
operation rather than one request per recording.
