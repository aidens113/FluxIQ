...
| [agent-token-efficiency-plan.md](./agent-token-efficiency-plan.md) | Senior supervisor agent | 62 | Core's share of the plan to keep the senior supervisor agent's context small on every project: a `CLAUDE.md` that loads `AGENTS.md`, the mirrored delegation rule and worker return contract, and the `working-docs` audit extension that Core owns. | `agent-token-efficiency-plan.md` |
| [agent-working-doc-protocol.md](./agent-working-doc-protocol.md) | Senior supervisor agent | 495 | How the supervisor and workers use `docs/working/` as durable memory and as the coordination substrate for multi-agent work. | `agent-working-doc-protocol.md` |
...
⚠ marks documents over the 800-line compaction threshold (16 of 26 here).
