...
| [flow-authoring-and-defensive-runtime-plan.md](./flow-authoring-and-defensive-runtime-plan.md) | Senior supervisor agent | 125 | Core’s share of making a model-authored Flow faithful to what the model actually did, and of making a Flow’s execution survive a site nobody controls: the build loop that explores by running real registry nodes, the accruing draft and its edit and dry-run seams, the per-node retry and recovery ladder that reads recorded state, and the permission gate a Flow’s own steps must pass. | `flow-authoring-and-defensive-runtime-plan.md` |
...
⚠ marks documents over the 800-line compaction threshold (16 of 30 here).
